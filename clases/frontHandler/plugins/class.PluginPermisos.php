...
{
    const MENSAJE_SE_SOLICITA_LOGIN = 'Debes autentificarte para acceder a esta accion';
    const MENSAJE_ACCION_DENEGADA = 'No tienes permiso para procesar la accion solicitada';
    
    /**
     * Esta funcion es para redirecciones que no sean desde peticiones Ajax.
     *
     * Importante: Conceptualmente SOLO se establece una nueva ruta,
    ...
    private function redireccionarRuta($mensajeAccionDenegada = self::MENSAJE_ACCION_DENEGADA, $mensajeSolicitaLogin = self::MENSAJE_SE_SOLICITA_LOGIN)
    {
        if(!SessionAutentificacion::getInstance()->realizoLogin()){
            //si no es ajax y no hizo login redirecciono a login
            ...
            list($modulo, $controlador, $accion) = SessionAutentificacion::getInstance()->obtenerIdentificacion()->getUrlRedireccion401();
    ...
    public function preDispatch(HttpRequest $request)
    {
        ...
        {
            //si es ajax devuelvo codigo 401 -> el javascript despues se encarga del mensaje. la accion esta en controlador Index.
            if($request->isXmlHttpRequest())
            {
                $request->setModuleName('index')
                        ->setControllerName('index')
                        ->setActionName('ajaxError')
                        ->setParam('msgInfo', 'Permiso denegado al realizar la accion')
                        ->setParam('codigoError', '401');
                return;
            }
