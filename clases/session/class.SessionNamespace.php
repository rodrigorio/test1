<?php
/**
 * SessionNamespace. Fijarse en la documentacion como usar los namespaces. Encapsula el uso de $_SESSION
...
class SessionNamespace extends SessionAbstract implements IteratorAggregate
{
    ...
    /**
     * Namespace - which namespace this instance of session is saving-to/getting-from
     *
     * @var string
     */
     ...
     * @var array
     */
    protected static $namespaceLocks = array();
     ...
     * @var array
     */
    protected static $singleInstances = array();
    ...
    public static function resetSingleInstance($namespaceName = null)
    {
        if ($namespaceName != null) {
            if (array_key_exists($namespaceName, self::$singleInstances)) {
                unset(self::$singleInstances[$namespaceName]);
            }
            return;
        }

        self::$singleInstances = array();
        return;
    }
    ...
    public function __construct($namespace = 'Default', $singleInstance = false)
    {
        ...
        if (isset(self::$singleInstances[$namespace])) {
        ...
        if ($singleInstance === true) {
            self::$singleInstances[$namespace] = true;
        }
                ...
                if (isset($_SESSION['__ZF'][$namespace]['ENNH'])) {
                    ...
                    if ($_SESSION['__ZF'][$namespace]['ENNH'] === 0) {
                        if (isset($_SESSION[$namespace])) {
                            self::$expiringData[$namespace] = $_SESSION[$namespace];
                    ...
                    foreach ($_SESSION['__ZF'][$namespace]['ENVNH'] as $variable => $hops) {
                        ...
                        if ($_SESSION['__ZF'][$namespace]['ENVNH'][$variable] === 0) {
                            if (isset($_SESSION[$namespace][$variable])) {
                                self::$expiringData[$namespace][$variable] = $_SESSION[$namespace][$variable];
    ...
    {
        return new ArrayObject(parent::namespaceGetAll($this->namespace));
    }
    ...
    {
        self::$namespaceLocks[$this->namespace] = true;
    }
    ...
    {
        unset(self::$namespaceLocks[$this->namespace]);
    }
    ...
    {
        return isset(self::$namespaceLocks[$this->namespace]);
    }
    ...
    {
        return parent::namespaceUnset($this->namespace);
    }
    ...
    public function & __get($name)
    {
        ...
        return parent::namespaceGet($this->namespace, $name);
    }
    ...
    {
        if (isset(self::$namespaceLocks[$this->namespace])) {
        ...
        if (parent::$writable === false) {
            throw new SessionException(parent::THROW_NOT_WRITABLE_MSG);
        }
    ...
    public function apply($callback)
    {
        ...
        $arg_list[0] = $_SESSION[$this->namespace];
    ...
    public function applySet($callback)
    {
        ...
        $_SESSION[$this->namespace] = $result;
        return $result;
    }
    ...
    public function __isset($name)
    {
        ...
        return parent::namespaceIsset($this->namespace, $name);
    }
    ...
    public function __unset($name)
    {
        ...
        return parent::namespaceUnset($this->namespace, $name);
    }
    ...
    public function setExpirationSeconds($seconds, $variables = null)
    {
        ...
        if ($variables === null) {
            ...
            $_SESSION['__ZF'][$this->namespace]['ENT'] = time() + $seconds;
        } else {
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    $_SESSION['__ZF'][$this->namespace]['ENVT'][$variable] = time() + $seconds;
                }
            }
        }
    }
    ...
    public function setExpirationHops($hops, $variables = null, $hopCountOnUsageOnly = false)
    {
        ...
        if ($variables === null) {
            ...
            if ($hopCountOnUsageOnly === false) {
                $_SESSION['__ZF'][$this->namespace]['ENGH'] = $hops;
            } else {
                $_SESSION['__ZF'][$this->namespace]['ENNH'] = $hops;
            }
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    if ($hopCountOnUsageOnly === false) {
                        $_SESSION['__ZF'][$this->namespace]['ENVGH'][$variable] = $hops;
                    } else {
                        $_SESSION['__ZF'][$this->namespace]['ENVNH'][$variable] = $hops;
                    }
                }
            }
        }
