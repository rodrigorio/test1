...
class MYSQL{
	...
	* @param $bAutoCommit
	*/
	public function __construct($host=null,$db=null,$user=null,$pw=null,$bAutoCommit=true){
		if( !is_null($host) &&
		!is_null($db) &&
		!is_null($user) &&
		!is_null($pw)){
	...
	*/
	 ...
	 * @return object
	 */
	public function oNextRecord($iNum = 0){
		if(!is_resource($this->QUERY_ID[$iNum])){
			return false;
		}
		return mysql_fetch_object($this->QUERY_ID[$iNum]);
	}
