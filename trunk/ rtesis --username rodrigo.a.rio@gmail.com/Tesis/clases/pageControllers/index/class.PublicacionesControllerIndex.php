...
{		
    ...
    {
        //Aca deveria levantar meta tags y title para la vista desde la DB
        $this->getTemplate()->load_file_section("gui/vistas/publicacion-ampliada.gui.html", "headContent", "HeadBlock");
        $this->getTemplate()->set_var("pathUrlBase", $this->getRequest()->getBaseTagUrl());
        $this->getTemplate()->set_var("sTituloVista", "Publicaci&oacute;n");
        return $this;
    }

    private function setCabeceraTemplate()
    {
        //estos en realidad tienen q venir de parametros
        $this->getTemplate()->set_var("sourceLogoHeader", 'sitio/uploads/design/logo.jpg');
        $this->getTemplate()->set_var("heightLogoHeader", '89');
        $this->getTemplate()->set_var("widthLogoHeader", '156');
        $this->getTemplate()->set_var("tituloHeader", "Titulo Blog");
        $this->getTemplate()->set_var("subtituloHeader", "Subtitulo, keyword1, keyword2");
        return $this;
    }

    private function setMenuTemplate()
    {
        $this->getTemplate()->load_file_section("gui/componentes/menues.gui.html", "menuHeader", "MenuHorizontalBlock");
    ...
    public function ampliarPublicacion(Publicacion $publicacion = null)
    {
        ...
        if(!empty($cantidadCriticas) && $activarValoracion){
            $this->getTemplate()->load_file_section("gui/vistas/publicacion-ampliada.gui.html",
