...
insert  into `areas`(`id`,`ciclos_id`,`descripcion`) values (1,1,'matematica'),(2,1,'sociales');
...
insert  into `diagnosticos`(`id`,`descripcion`) values (1,'kjnjknjk'),(2,NULL);
...
insert  into `diagnosticos_scc`(`id`) values (1),(2);
...
insert  into `diagnosticos_scc_x_ejes`(`diagnosticos_scc_id`,`ejes_id`,`estadoInicial`) values (1,2,'		11111123123\n		'),(1,3,'asdasdsad');
...
/*Table structure for table `entrevista_x_usuario` */

DROP TABLE IF EXISTS `entrevista_x_usuario`;

CREATE TABLE `entrevista_x_usuario` (
  `entrevistas_id` int(11) NOT NULL,
  `seguimientos_id` int(11) NOT NULL,
  PRIMARY KEY (`entrevistas_id`,`seguimientos_id`),
  KEY `FK_entrevista_x_seguimiento_usuario` (`seguimientos_id`),
  CONSTRAINT `FK_entrevista_x_seguimiento_usuario` FOREIGN KEY (`seguimientos_id`) REFERENCES `seguimientos` (`id`) ON DELETE CASCADE,
  CONSTRAINT `FK_entrevista_x_seguimiento` FOREIGN KEY (`entrevistas_id`) REFERENCES `entrevistas` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `entrevista_x_usuario` */
...
/*Table structure for table `pregunta_x_preguntas_opcion` */

DROP TABLE IF EXISTS `pregunta_x_preguntas_opcion`;

CREATE TABLE `pregunta_x_preguntas_opcion` (
  `preguntas_id` int(11) NOT NULL,
  `preguntas_opciones_id` int(11) NOT NULL,
  PRIMARY KEY (`preguntas_id`,`preguntas_opciones_id`),
  KEY `FK_pregunta_x_preguntas_opcion__x_opciones` (`preguntas_opciones_id`),
  CONSTRAINT `FK_pregunta_x_preguntas_opcion` FOREIGN KEY (`preguntas_id`) REFERENCES `preguntas` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `pregunta_x_preguntas_opcion` */
...
CREATE TABLE `preguntas_opciones` (
  ...
  PRIMARY KEY (`preguntas_id`,`opciones_id`),
...
CREATE TABLE `preguntas_simples_respuestas` (
  ...
  CONSTRAINT `FK_preguntas_simples_respuestas_seguimientos` FOREIGN KEY (`seguimientos_id`) REFERENCES `seguimientos` (`id`),
  CONSTRAINT `FK_preguntas_simples_respuestas` FOREIGN KEY (`preguntas_id`) REFERENCES `preguntas` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
insert  into `seguimientos`(`id`,`discapacitados_id`,`frecuenciaEncuentros`,`diaHorario`,`practicas_id`,`usuarios_id`,`antecedentes`,`pronostico`,`fechaCreacion`,`estado`) values (1,122,'todos los dias','ads',2,61,NULL,NULL,'2013-01-08 19:28:56','activo'),(2,122,'todos los dias','asdsa',1,61,'AAAAaaaaAAa AAAa aaaaa...',NULL,'2013-01-08 19:32:57','activo');
...
insert  into `seguimientos_scc`(`id`,`diagnosticos_scc_id`) values (1,1),(2,1);
...
insert  into `unidades`(`id`,`usuarios_id`,`nombre`,`descripcion`,`preCargada`,`fechaHora`,`asociacionAutomatica`) values (1,NULL,'unidad por defecto','Variables de texto asignadas al seguimiento al crearse.',0,NULL,1),(5,63,'Una nueva Unidad','esta es la primera unidad de variables de prueba en el sistema',0,'2013-04-07 04:39:43',0),(6,63,'haciendo una prueba','aldkjasldkj asdl jasldkj askdja s\ndasldj alsdkjasldj aldsj asldj ',0,'2013-04-07 07:25:21',0),(7,63,'una nueva unidad te queres matar chee','sdfkjahdsfkljhasdkjfhasldkfhd\nfasdfkjhasdfkj',0,'2013-04-11 00:04:00',0);
...
CREATE TABLE `variable_cualitativa_modalidades` (
  ...
  CONSTRAINT `FK_variable_modalidades` FOREIGN KEY (`variables_id`) REFERENCES `variables` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

/*Data for the table `variable_cualitativa_modalidades` */

insert  into `variable_cualitativa_modalidades`(`id`,`variables_id`,`modalidad`,`orden`,`borradoLogico`) values (1,5,'malo',1,0),(2,5,'regular',1,0),(3,5,'bueno',1,0);
...
CREATE TABLE `variables` (
  ...
  CONSTRAINT `FK_variables_unidades` FOREIGN KEY (`unidad_id`) REFERENCES `unidades` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

/*Data for the table `variables` */

insert  into `variables`(`id`,`nombre`,`tipo`,`descripcion`,`unidad_id`,`fechaHora`,`borradoLogico`) values (1,'Texto descripcion','VariableTexto',NULL,1,NULL,0),(3,'Una prueba','VariableTexto','asdas\r\ndasd\r\nasd\r\nasdasdasdas',6,'2013-04-07 07:26:19',0),(4,'adsfadsf 1234','VariableNumerica','adfsadsfads 1234',5,'2013-04-07 07:26:58',0),(5,'sdfasdfasdfadsf','VariableCualitativa','adsfasdfadsf',5,'2013-04-07 07:27:08',0),(6,'una nueva variable de texto1','VariableTexto','asdlkasjdlas\nasldjasldjasldjlakdjalsdjalsdj\nasldjasldkjaskld\n\nalskdjalsdjalskdjlaskjdlaskd1',5,'2013-04-11 02:42:03',0),(7,'nueva variable ','VariableTexto','adsfkladsjf lakdsj fadsf\ndasflj adslfkj adsf\nadlfkj adslfjadlskjf \ndalfkj adslfk aldskjf ads\n',5,'2013-04-11 03:03:27',0),(8,'nueva variable numerica','VariableNumerica','sdfads',5,'2013-04-11 03:08:08',0);
